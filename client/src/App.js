...
import { OPT, DoctorAddRecord } from "./Components/contracts";
...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      tmpcont["OPT"] = new web3.eth.Contract(optHealthCare.abi, OPT);

      var deployedNetworks = docAddRecord.networks[networkId];

      tmpcont["DAR"] = new web3.eth.Contract(docAddRecord.abi, DoctorAddRecord);

      console.log("contract for both doctor and opt is----->", tmpcont);
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        ) : this.state.loggedas === 1 ? (
          <Patient contract={this.state.contract} Acc={this.state.accounts} />
        ) : this.state.loggedas === 2 ? (
          <Hospital contract={this.state.contract} Acc={this.state.accounts} />
        ) : this.state.loggedas === 3 ? (
