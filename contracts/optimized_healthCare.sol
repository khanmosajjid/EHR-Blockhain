...
contract optimized_healthCare {
    ...
    function signupPatient(
        ...
        require(!(p.id > address(0x0)),"address should not be zero address");
        //Add patient to blockchain
        require(a.adhaar_number > 0,"Adhar number cannot be zero");
        ...
        checkDoctor(doctor_id)
    {
        ...
        uint16 pos = uint16(p.doctor_list.length); // new length of array
    ...
    function revokeAccessFromDoctor(address doctor_id)
        public
        checkPatient(msg.sender)
    {
    ...
    function checkDoctorInfo(uint64 _adhaar_number)
        public
        view
    ...
    function checkPatientInfo(uint64 _adhaar_number)
        public
        view
        returns (bool, address)
    {
    ...
    function hospitalGrantAccess(address _user, address _patient)
        public
    ...
    function regInsuranceComp(address _id, string memory _name)
        public
        onlyOwner
    {
        insuranceComp storage i = insuranceCompanies[_id];

        // Check if the insurance company is not already registered
        // The zero address (0x0) is typically used to indicate an uninitialized or default state
        require(i.id == address(0x0), "Insurance company already registered");

        // Register the new insurance company
        insuranceComp storage newInsuranceComp = insuranceCompanies[_id];
        newInsuranceComp.id = _id;
        newInsuranceComp.name = _name;
        // Initialize an empty array of registered patients
        newInsuranceComp.regPatients = new address[](0);
    }
    ...
    {
        insuranceComp storage i = insuranceCompanies[msg.sender];
        return (i.id, i.name, i.regPatients);
    }

    function addPatientToInsuranceComp(address _insuranceComp, address _pat)
        public
    {
        insuranceComp storage i = insuranceCompanies[_insuranceComp];
        require(i.regPatientsMapping[_pat] < 1);

        i.regPatients.push(_pat);
        uint8 pos = (uint8)(i.regPatients.length);
